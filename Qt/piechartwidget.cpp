...
PieChartWidget::PieChartWidget(const DipintoSet *dipinti, QWidget *parent)
    : QWidget(parent) {
...
void PieChartWidget::updateChart(const DipintoSet *dipinti) {
  ...
  // Prende solo i primi 10 elementi, rimuovi gli altri dal vettore e somma i
  // loro valori
  ...
  for (int i = 0; i < slices.size(); ++i) {
    ...
    QPieLegendMarker *marker =
        qobject_cast<QPieLegendMarker *>(chart->legend()->markers(series)[i]);
    if (marker) {
      marker->setLabel(
          QString("%1 (%2%)").arg(label).arg(slices[i]->percentage()));
    }
  }
...
QSize PieChartWidget::sizeHint() const { return QSize(500, 500); }
