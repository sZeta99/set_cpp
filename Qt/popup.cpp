...
void PopUp::initializeComponents()
{
    ...
    connect(aggiungiDipintoButton, &QPushButton::clicked, this, &PopUp::onAggiungiDipintoClicked);
}
...
void PopUp::onAggiungiDipintoClicked()
{
    ...
    if (scuola.isEmpty() || autore.isEmpty() || soggettoTitolo.isEmpty() || data.isEmpty() || sala.isEmpty())
    {
        QMessageBox::warning(this, "Errore", "Compila tutti i campi per aggiungere un dipinto.");
        return;
    }
    ...
    dipinto d = dipinto(scuola.toStdString(), autore.toStdString(), soggettoTitolo.toStdString(),
                        data.toStdString(), sala.toStdString());
    ...
    if (!added)
    {
        QMessageBox *errorDialog = new QMessageBox(QMessageBox::Critical, "Message", "Item Già Presente", QMessageBox::Ok, this);
