...
public:
    ...
    inline bool operator()(const dipinto &e) const { return (e.data.substr(0, 2) == this->data.substr(0, 2)); }
...
BarChartWidget::BarChartWidget(const DipintoSet *dipinti, QWidget *parent) : QWidget(parent)
{
...
void BarChartWidget::updateChart(const DipintoSet *dipinti)
{
    ...
    for (const auto &s : sortedDipinti)
    {
        ...
        *barSet << d.size();
        barSet->setLabel(QString::fromStdString(s) + " (" + QString::number(d.size()) + ")");
        barSet->setBrush(barColors.at(barSeries->barSets().size() % barColors.size()));
    ...
    // Ridimensione del font in base alla dimensione della finestra e al numero di secoli mostrati
    int numLegendFields = chart->legend()->markers().count();
    if(numLegendFields >0){
    int area = parentWidget()->width() * parentWidget()->height();
    int fontSize = static_cast<int>(qSqrt(area) / (15 * qMax(5, numLegendFields)));
