...
void populateTableFromSet(QTableWidget &table, const DipintoSet &dipintiSet) {
  ...
  for (const auto &dipinto : dipintiSet) {
    table.setItem(row, 0,
                  new QTableWidgetItem(QString::fromStdString(dipinto.scuola)));
    table.setItem(row, 1,
                  new QTableWidgetItem(QString::fromStdString(dipinto.autore)));
    table.setItem(row, 2,
                  new QTableWidgetItem(
                      QString::fromStdString(dipinto.soggetto_o_titolo)));
    table.setItem(row, 3,
                  new QTableWidgetItem(QString::fromStdString(dipinto.data)));
    table.setItem(row, 4,
                  new QTableWidgetItem(QString::fromStdString(dipinto.sala)));
